...
| Qt-Radio am Mittelpunkt des breiten Label-Rechtecks klicken | Label reagiert nicht; fachlicher Zustand bleibt trotz physischem Klick unverändert | nur den linken Indikatorbereich anklicken und danach die gesamte exklusive Radiogruppe rücklesen |
| `expectedPageAfter` bei `pattern=select` als Navigation behandeln | korrekte Auswahl auf derselben Seite wird als Navigationsfehler gemeldet | `select` aus der Navigationsnachbedingung ausnehmen; Seitenstabilität und Gruppenstatus separat prüfen |
| nummerierte Fahrzeugüberschrift starr auf `1. Fahrzeug` oder `2. Fahrzeug` binden | weitere Fahrzeuge oder eine nachgetragene Bezeichnung brechen Page Objects | `N. <Label>` und `N. <Label>: <Detail>` semantisch über `headingNumberedLabel` binden |
| langen Tabellen-Combo-Inhalt nur einmal nach sichtbaren Optionen lesen | virtualisierte Kategorie wie `Sonst. Leistung EU` existiert, ist aber nicht materialisiert | Liste PID-/Root-gebunden seitenweise scrollen, Ziel nach jedem Schritt frisch binden und erst dann auswählen |
| leere Tabellenzeile nur über sichtbaren Zelltext `0,00` löschen | mehrere identische Zellen binden die falsche Zeile | fachliche Zeilenidentität aus Runtime-ID, Schlüsselspalten und Struktur-Fingerprint bilden; Nachsumme und Zeilenzahl prüfen |
| mehrere SSE-MCP-Aufrufe parallel senden | der zweite Aufruf endet erwartbar mit `busy`, weil der Desktop-Worker seriell ist | Aufrufe pro SSE-Instanz strikt seriell planen; Parallelität nur für unabhängige Datei-/Webprüfungen nutzen |
| Legacy-`expectedPath` direkt zum Speichern verwenden | Ressourcen- und Übermittlungsschutz der MCP-Schicht kann umgangen werden | Übermittlungsstatus auch im Worker unmittelbar vor dem Save fail-closed prüfen; Ausnahme nur für eine separat benannte Korrektur-/Berichtigungskopie mit Freigabe, Zeitraum, Grund sowie hashgebundenem übermittelten Original und bytegleicher Vorzustands-Sicherung; kein generisches `force` |
| Fall außerhalb des konfigurierten `caseDir` nur im Fenstertitel binden | MCP-`caseRef` kann Hash/Save nicht adressieren, während Legacy-Pfade weiterreichen | externen Fall explizit in einen hashverifizierten Working-Copy-Workflow übernehmen oder einen gleichwertigen Ressourcenvertrag ergänzen |
